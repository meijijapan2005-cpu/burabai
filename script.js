...
const grid=document.querySelector("#grid"),q=document.querySelector("#q"),empty=document.querySelector("#empty");
let filter="";
function render(){const term=q.value.trim().toLowerCase();const a=products.filter(x=>(!filter||x.cat===filter)&&(`${x.cat} ${x.name} ${x.shop}`.toLowerCase().includes(term)));grid.innerHTML=a.map(x=>`<article class="product"><div class="visual"><span class="discount">${x.off}</span><span class="demo">DEMO</span><i>${x.icon}</i></div><div class="body"><small>${x.cat}</small><h3>${x.name}</h3><span class="price">${x.price}</span><span class="old">${x.old}</span><div class="merchant">${x.shop}</div><button class="visit" disabled title="提携ショップ接続後に有効になります">ショップで見る ↗</button></div></article>`).join("");empty.style.display=a.length?"none":"block"}
...
document.querySelector("#go").onclick=render;q.addEventListener("keydown",e=>{if(e.key==="Enter")render()});
document.querySelectorAll("[data-filter]").forEach(b=>b.onclick=()=>choose(b.dataset.filter));
document.querySelector("#all").onclick=()=>{q.value="";choose("")};
